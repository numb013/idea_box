<html>
  ...
  if (res == true) {ldelim}
    document.approval_form.action = 'status.php?mode=status&approval=3&send=group_non_admitted_list&number={$page|escape}';
  ...
  if (res == true) {ldelim}
    location.href = "{$smarty.const.URL_ROOT_HTTPS}/admin/delete.php?mode=sakujyo&number={$page|escape}&send=group_non_admitted_list&group_id=" + group_id;
  {rdelim}
  {rdelim}
  function sakujyo() {ldelim}
    res = confirm('削除してもよろしいでしょうか？');
  if (res == true) {ldelim}
    document.approval_form.action = 'delete.php?mode=sakujyo&send=group_non_admitted_list&number={$page|escape}';
    document.approval_form.submit()
  {rdelim}
  {rdelim}
  ...
  if (res == true) {ldelim}
    location.href = "{$smarty.const.URL_ROOT_HTTPS}/admin/member_csv.php?mode=download&?approval_status={$approval_status|escape}&group_id=" + group_id;
  {rdelim}
  {rdelim}
        ...
        <form action="approval.php" method="POST" name="approval_form">
          <table class="table" width="95%">
            ...
            {foreach item="group_map" from=$group_list key=item_key}
              <tr>
                <td><center><input type="checkbox" name="group_check[]" value={$group_map.group_id|escape}></center></td>
                <td><center><a href="{$smarty.const.URL_ROOT_HTTPS}/admin/group_member_list.php?group_id={$group_map.group_id}&approval_status={$group_map.approval_status|escape}&number={$page|escape}">{$group_map.group_name|escape}</a></center></td>
                ...
                <td><center><a href="#" onclick="sakujyo_link('{$group_map.group_id}')">削除</a></center></td>
                <td><center><a href="#" onclick="one_member_csv('{$group_map.group_id|escape}'); return false;">DL</a></center></td>
                <td><center>{$statustypeDef->getSelectTags("group_status[]", 90, $group_map.approval_status, "")}</center></td>
              </tr>
