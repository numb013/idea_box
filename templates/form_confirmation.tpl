...
<body>
  <!-div id="home" data-role="page">
    ...
    <div data-role="content">	
      ...
      <dl class="form">
      <dt>社名</dt>
      <dd>{$input_map.company|escape}</dd>
      <dt>氏名</dt>
      <dd>{$input_map.member_name|escape}</dd>
      <dt>研修日</dt>
      <dd>{$input_map.training_datetime|escape}</dd>
      <dt>研修コード</dt>
      <dd>{$trainingcodeDef->getStringByValue($input_map.training_code)|escape}</dd>
      <dt>テーブルID</dt>
      <dd>{$tablecodeDef->getStringByValue($input_map.table_code)|escape}</dd>
      <dt>メールアドレス</dt>
      <dd>{$input_map.mail_address|escape}</dd>
      <dt>備考テキスト</dt>
      <dd>{$input_map.memo|escape|nl2br}</dd>
      </dl>
      <br><br>
